$(function() {
	...
	function runWebLayout() {

		//声明desktop空间,封装相关操作
		myLib.NS("desktop");
		myLib.desktop = {
			//给body设置数据，存储window的宽高值
			winWH: function() {
				$('body').data('winWh', {
					'w': $(window).width(),
					'h': $(window).height()
			...
			desktopPanel: function() {
				$('body').data('panel', {
					//底部任务栏
					'taskBar': {
						'_this': $('#taskBar'), //底部任务栏面板
						'task_lb': $('#task_lb') //底部任务栏面板的选项卡标题
					},
					//左边侧边栏
					'lrBar': {
						'_this': $('#lr_bar'), //左边侧边栏的box
						'default_app': $('#default_app'), //左边侧边栏的list
						'start_block': $('#start_block'), //左边侧边栏底部模块的设置菜单模块
						'start_btn': $('#start_btn'), //左边侧边栏底部模块的设置菜单主要操作按钮
						'start_item': $('#start_item'), //左边侧边栏底部模块的设置菜单主要列表
						'default_tools': $('#default_tools')
					},
					//桌面应用图标
					'deskIcon': {
						'_this': $('#deskIcon'), //桌面应用列表
						'icon': $('li.desktop_icon') //桌面应用列表的单个组件
					},
					'powered_by': $('a.powered_by') //版权
				});
			},
			//获取body存储的数据
			getMydata: function() {
			...
			/**
			 * 上下文菜单，并监听body点击隐藏上下文菜单事件
			 * @param {Object} jqElem 绑定上下文菜单的jq选择器
			 * @param {Array} data 上下文菜单主要数据
			 * @param {String} menuName 上下文菜单容器的自定义id
			 * @param {Number} textLimit 上下文菜单项限制显示的文字个数。如果超出会截取，并以…补全，完成文字以title形式显示。
			 * */
			contextMenu: function(jqElem, data, menuName, textLimit) {
				...
				jqElem.smartMenu(data, {
					name: menuName,
					textLimit: textLimit,
					afterShow: function() { //设置上下文菜单容器相对于被绑定的jq选择器的水平和垂直距离
		...
		myLib.desktop.win = {
			 ...
			 * @param {String} url 子页面地址
			 * @param {String} id 子页面的id
			 * */
			...
			hyimizeWin: function(obj) {
				...
				obj.css({
					...
					.find(".winframe")
					.css({
						'width': winLocation['w'] - 6,
						'height': winLocation['h'] - 29
					});
			},
			//交换窗口z-index值		
			...
			newWin: function(options) {
				...
				var defaults = {
					...
					/* Posible are pixels or 'center' */
					WindowWidth: Math.round(wh['w'] * 0.8),
//					WindowWidth: Math.round(wh['w'] * 1),
					/* Only pixels */
					WindowHeight: Math.round(wh['h'] * 0.8),
//					WindowHeight: Math.round(wh['h'] * 1),
					/* Only pixels */
					WindowMinWidth: 250,
					/* Only pixels */
					WindowMinHeight: 250,
...
//					WindowStatus: 'maximized',
				...
				if($("#" + id).size() <= 0) {
					...
					$newWin
						...
						.find(".winframe")
						.css({
							"width": options['WindowWidth'] - 6,
							"height": options['WindowHeight'] - 29
						})
						...
						$default_app
							...
							.find('img').click(function() {
								...
								if(window.screen.width <= 1024) {
									if($("#task_lb>a").length < 10) {
										var title = $(this).attr('title'),
											wid = $(this).parent().attr('id');
									...
									} else if($("#task_lb>a").length >= 10) {
										console.log($("#task_lb>a").length);
										layer.alert("能打开的子窗口只有10个，请关闭多余的窗口再打开！", {
											icon: 0
										});
									}
								}
		...
		myLib.desktop.deskIcon = {
			...
			arrangeIcons: function() {
				...
				//console.log($('body').data('deskIconNum'));
			...
			init: function() {
				...
				$icon.hover(function() {
					...
					.click(function() {
						...
						if(window.screen.width <= 1024) {
							if($("#task_lb>a").length < 10) {
								var title = $(this).children("div.text").text(),
									wid = this.id;
								var href = this.getAttribute("path");
								myLib.desktop.win.newWin({
									WindowTitle: title,
					...
					.droppable({
						drop: function() {
							...
							var l = $(this).css('left'),
								t = $(this).css('top');
							$(this).css({
								'left': curDragIcon.css('left'),
								'top': curDragIcon.css('top')
							});
							curDragIcon.css({
								'left': l,
								'top': t
